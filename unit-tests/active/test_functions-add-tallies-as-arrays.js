...
const exampleVisitorProfileNoTallies = {
  transactionId: '583531ec-0b67-4309-8a9d-e673aa0f6346',
  live: false,
  visitor: {
    metrics: {
...
describe('the addTallies function, testing for use in Tealium Functions', function () {
  ...
  it('should work correctly with tallies', function () {
    const result = exported.addTalliesToEventPayload({ test_existing_key: true }, exampleVisitorProfile.visitor)
  ...
  it('should work correctly without tallies', function () {
    const result = exported.addTalliesToEventPayload({ test_existing_key: true }, exampleVisitorProfileNoTallies.visitor)
    chai.expect(result).to.deep.equal({
      test_existing_key: true
    })
  })
