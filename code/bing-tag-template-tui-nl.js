//~~tv:2063.20201020
//~~tc: Update event action handling in events which can send empty string

var uetq = uetq || [];

//tealium universal tag - utag.sender.2063 ut4.0.##UTVERSION##, Copyright ##UTYEAR## Tealium.com Inc. All Rights Reserved.
try {
  (function (id, loader) {
    var u = {"id" : id};
    ...
    u.event_map = {
      ...
      "sign_up": [{ "name":"method" } ],
      "timing_complete": [{ "name":"name" }, { "name":"event_value" }, { "name":"currency" }],
      
      // (CUSTOM) Added variable revenue event for TUI NL - https://help.ads.microsoft.com/apex/index/3/en/56687
      "variable_revenue": [{ "name":"currency" }, { "name":"revenue_value" } ],
    ...
    u.std_params = {
      ...
      "event_label" : function(_event) {
        if (u.data[_event] && u.data[_event].order_subtotal){
          return u.data[_event].order_subtotal;
        } else {
          return u.data.order_subtotal;
        }
    ...
    u.loader_cb = function (a, b, c) {
      ...
      for (i = 0; i < u.data.event.length; i++) {
        ...
        case "variable_revenue" : 
          data = {
            revenue_value: u.data.eventData[_event].revenue_value,
            currency: u.data.eventData[_event].currency,
          };
          for(keyname in customEventData){
            data[keyname] = customEventData[keyname];
          ...
          u.clearEmptyKeys(data);
          window.uetq.push ("event", "", data); // the empty string as the second arguement is strange but correct
          break;
        case "view_item" :
        case "view_item_list" :
          data = {
