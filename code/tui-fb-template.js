    ...
    u.send = function (a, b) {
      if (u.ev[a] || u.ev.all !== undefined) {
        ...
        for (i = 0; i < u.data.event_queue.length; i++) {
          ...
          // CUSTOM - add in industry-specific parameters unconditionally if provided (for non-pageview events)
          var industryParams = {
            "availability" : "",
            "body_style" : "",
            "checkin_date" : "",
            ...
            "user_score" : "",
            "year" : "",
            // add these too, since it needs to be included
            "content_type" : "",
            "content_name" : "",
            "content_ids" : "",
            // this one is CUSTOM for TUI it seems?
            "haulType" : ""
          }
          if (event_name !== "PageView") {
            for (key in industryParams) {
              if (industryParams.hasOwnProperty(key)) {
                event_data[key] = u.data[key];
              }
            }
            // force content_ids to an array if it's a string
            if (typeof event_data.content_ids === "string") {
              event_data.content_ids = event_data.content_ids.split(',')
            }
          }
